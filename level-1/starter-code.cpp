 ...
 */
const std::string VERSION = "0.4.<25022950>";
...
const int RANDOM_SEED = std::random_device{}();
...
void displayBoard(const char board[][BOARD_N_MAX], const int size) {
    for (int i = 0; i < size+3; i++)
    {
        ...
        {
            if (i==0) {if (j == 0) std::cout << "   0   ";else if (j == size) std::cout << "\n"; else std::cout << ((j < 10) ? std::format("{}   ", j) : std::format("{}  ", j));}
            else if (i==1 || i==size+2) {std::cout << "  " << std::string(size*4-1, '-') << "\n"; break;}
            else
            {
                if (j == 0) std::cout << ((i-2 < 10) ? std::format("{} |",i-2) : std::format("{}|",i-2));
                else if (j == size) {if (i == size+1) {std::cout << board[i-2][j-1] << "|\n";} else {std::cout << board[i-2][j-1] << "|\n\n";};}
...
GameResult playGame(const RunConfig& config,
    ...
    result.isBot = false; 
    pII move;
    bool botPlayer = (gameSetup.mode!=GameMode::PVP);
    if (config.interactive)
    {
    ...
    while(true)
    {
        if ((gameSetup.mode == GameMode::PVE && turns % 2 == 1) || gameSetup.mode == GameMode::EVE)
        {
            if (gameSetup.mode == GameMode::EVE) (currentLevel = currentPlayer);
            move = botMove(gameSetup.board, 
                gameSetup.size, 
                gameSetup.goal, 
                symbols[currentPlayer], 
                gameSetup.levels[currentLevel]);
            
            auto result = measureExecutionTime("botMove",
        ...
        else
        {
            ...
            while (!getPlayerMove(&move.first,&move.second) || !isValidMove(gameSetup.board, gameSetup.size, move.first, move.second))
            {
        ...
        if (config.interactive)
        {
            ...
            std::cout << std::format("Player {}'s turn\n", symbols[1 - currentPlayer]);
        }
        
        if (checkWin(gameSetup.board, 
                    gameSetup.size,
                    symbols[currentPlayer],
                    gameSetup.goal, EndRule::OPEN_TWO))
        {
            result.winner = currentPlayer;
            result.turns = turns + 1;
            result.isBot = botPlayer && currentPlayer;
            return result;
        }
...
void endGame(const RunConfig& config,
    ...
    if (config.interactive)
    {
        ...
        GameLogger::log(std::format("Player {} wins ", gameResult.winner + 1) + std::format("after {} turns, ", gameResult.turns)+ std::format("Is winner a bot? {}\n", gameResult.isBot ? "Yes" : "No"));
    }
    ...
    for (int i = 0; i < size; i++)
    {
        for (int j = 0; j < size; j++)
        {
            ...
            for (auto [dx, dy] : dir)
            {
                ...
                int x = j + dx;
                int y = i + dy;
                while (x > -1 && y > -1 && x < size && y < size && board[y][x] == symbol)
                {
                    length++;
                    if (length == goal) 
                    {
                        ...
                        {
                            if (isEmptyHead(board, size, x+dx, y+dy, symbol) || isEmptyHead(board, size, x-dx*length, y-dx*length, symbol))
                            {
                                return true;
                            }
                        }
                        ...
                        {
                            if (isEmptyHead(board, size, x+dx, y+dy, symbol) && isEmptyHead(board, size, x-dx*length, y-dy*length, symbol))
                            {
                                return true;
                            }
                        }
...
pII random_pick(char board[][BOARD_N_MAX],
                const int size) {
    // TODO: student implementation
    std::vector<pII> availibleMove = {};
    for (int i = 0; i < size; i++)
    {
        for (int j = 0; j < size; j++)
        {
            if (board[i][j] == '-') availibleMove.push_back({i,j}); 
        }
    }
    std::uniform_int_distribution<int> dist(0, availibleMove.size() - 1);
    int randomNum = dist(generator);
    // placeholder
    return availibleMove.at(randomNum);
}
...
pII simple_heuristic(char board[][BOARD_N_MAX],
    ...
    char current[2] = {botSymbol, playerSymbol};
    float mid = (size-1)/2.0f;
    std::priority_queue<std::tuple<double, double, double, pII> > availibleMove = {};
    for (int n = goal; n > 2; n--)
    {
        for (int k = 0; k < 2; k++)
        {
            for (int i = 0; i < size; i++)
            {
                for (int j = 0; j < size; j++)
                {
                    ...
                    float score = 0;
                    float distance = std::pow((i-mid),2)+std::pow((j-mid),2);
                    ...
                    if (board[i][j] == '-') 
                    {
                        ...
                        int noise = noiseDist(generator);
                        if (checkWin(board, size, current[k], n))
                        {
                            score = n + (0.5-k);
                            availibleMove.push({score, -distance, noise, {i, j}});
                        }
                        else
                        {
                            availibleMove.push({0, -distance, noise, {i, j}});
                        }
        ...
        std::tuple score_move = availibleMove.top();
        if (std::get<0>(score_move) > 0)
        {
            return std::get<3>(score_move);
        }
    }
    std::tuple score_move = availibleMove.top();
    return std::get<3>(score_move);
 ...
 */
bool hasNeighbour(char board[][BOARD_N_MAX], int size, int row, int col)
{
...
// int maxLength(char board[][BOARD_N_MAX],
//               const int size,
//               const char symbol,
//               const int goal) {
//     int maxLength = 1;
...
//                     y+=dy;
//                 }
//                 bool forwardObs = false;
//                 int l = 0;
//                 while (goal-length+1+l > 0)
//                 {
//                     forwardObs = forwardObs && isEmptyHead(board, size, x+dx*(goal - length + 1 + l), y+dy*(goal-length+1 + l), symbol);
//                     l--;
//                 }
//                 if (forwardObs && isEmptyHead(board, size, x-dx*length, y-dy*length, symbol))
//                 {
//                     length*=2;
//                 }
//                 else if (!forwardObs && !isEmptyHead(board, size, x-dx*length, y-dy*length, symbol))
//                 {
//                     length=1;
//                 }
...
// double score(char board[][BOARD_N_MAX], int size, int goal)
// {
//     if (checkDraw(board, size))
//     {
//         return 0;
//     }
//     else if (checkWin(board, size, 'X', goal))
//     {
//         return 1;
//     } else if (checkWin(board, size, 'O', goal))
//     {
//         return -1;
//     }
//     return (std::pow(maxLength(board, size, 'X', goal),2) - std::pow(maxLength(board, size, 'O', goal),2))/std::pow(2*goal,2);
// }
double score(char board[][BOARD_N_MAX], int size, int goal)
{
    if (checkWin(board, size, 'X', goal)) return 1.0;
    if (checkWin(board, size, 'O', goal)) return -1.0;
    if (checkDraw(board, size)) return 0.0;

    int xLen = maxLength(board, size, 'X');
    int oLen = maxLength(board, size, 'O');

    return (double)(xLen * xLen - oLen * oLen) / (goal * goal);
}

std::vector<pII> getMoves(char board[][BOARD_N_MAX], int size)
{
    std::vector<pII> moves;

    for (int i = 0; i < size; i++) {
        for (int j = 0; j < size; j++) {
            if (board[i][j] == '-' && hasNeighbour(board, size, i, j)) {
                moves.push_back({i, j});
            }
        }
    }
...
// double minimax(char board[][BOARD_N_MAX], double alpha, double beta, int size, int goal, int depth, bool maximizing_player)
// {
//     if (depth == 0 || checkWin(board, size, 'X', goal) || checkWin(board, size, 'O', goal) || checkDraw(board, size))
//     {
//         return score(board, size, goal);
//     }
//     std::vector<pII> availibleMove = {};
//     for (int i = 0; i < size; i++)
//     {
//         for (int j = 0; j < size; j++)
//         {
//             if (board[i][j] == '-' && hasNeighbour(board, size, i, j)) availibleMove.push_back({i,j}); 
//         }
//     }
//     if (maximizing_player)
//     {
//         double max_eval = -std::numeric_limits<double>::infinity();
//         for (auto m : availibleMove)
//         {
//             board[m.first][m.second] = 'X';
//             double eval = minimax(board, alpha, beta, size, goal, depth - 1, false);
...
//             double eval = minimax(board, alpha, beta, size, goal, depth - 1, true);
...
double minimax(char board[][BOARD_N_MAX],
               int size,
               int goal,
               int depth,
               double alpha,
               ...
               bool maximizing)
{
    if (depth == 0 || checkWin(board, size, 'X', goal) ||
        checkWin(board, size, 'O', goal) || checkDraw(board, size))
    {
        return score(board, size, goal);
    }

    auto moves = getMoves(board, size);

    std::vector<std::pair<pII, double>> scoredMoves;

    for (auto [r, c] : moves) {
        board[r][c] = maximizing ? 'X' : 'O';
        double quickScore = score(board, size, goal); 
        board[r][c] = '-';

        scoredMoves.push_back({{r, c}, quickScore});
    }

    std::sort(scoredMoves.begin(), scoredMoves.end(),
    [maximizing](auto &a, auto &b) {
        return maximizing ? a.second > b.second : a.second < b.second;
    });
    if (moves.empty()) return score(board, size, goal);

    if (moves.size() > 18) moves.resize(18);
    if (maximizing) {
        double best = -1e9;
        ...
        for (auto [r, c] : moves) {
            ...
            double val = minimax(board, size, goal,
...
//             if (board[i][j] == '-')
//             { 
//                 if (hasNeighbour(board, size, i, j)) availibleMove.push_back({i,j}); 
//             } else turns_played++;
//         }
//     }

//     double bestValue = (botSymbol == 'X') ? -std::numeric_limits<double>::infinity() : std::numeric_limits<double>::infinity();
//     std::tuple<pII,double> best = {{-1,-1}, bestValue};
//     std::tuple<pII,double> worse = {{-1,-1}, -bestValue};
//     if (availibleMove.empty())
//     {
//         int mid = (size-1)/2;
//         return {mid,mid};
//     }
//     else if (turns_played < 4)
//     {
//         return simple_heuristic(board, size, goal, botSymbol, playerSymbol);
//     }
//     for (auto m : availibleMove)
//     {
//         board[m.first][m.second] = botSymbol;

//         double score = minimax(board, 
//                                -std::numeric_limits<double>::infinity(), 
//                                std::numeric_limits<double>::infinity(), 
//                                size, goal, 4, botSymbol != 'X');
//         board[m.first][m.second] = '-';
        
//         if (botSymbol == 'X')
//         {
//             if (score > std::get<1>(best))
//             {
//                 best = {m,score};
//             }
//             if (score < std::get<1>(best))
...
//         }
//     }
//     return best != worse ? std::get<0>(best) : simple_heuristic(board, size, goal, botSymbol, playerSymbol);
// }
pII hard_level(char board[][BOARD_N_MAX],
               int size,
               int goal,
               ...
               char playerSymbol)
{
    ...
    if (turns < goal*2 && !(size < 4)) {
        return simple_heuristic(board, size, goal, botSymbol, playerSymbol);
    }

    double bestVal = (botSymbol == 'X') ? -1e9 : 1e9;
    pII bestMove = moves[0];

    for (auto [r, c] : moves) {
        board[r][c] = botSymbol;

        double val = minimax(board,
                             size,
                             goal,
                             7,
                             -1e9,
        ...
        if ((botSymbol == 'X' && val > bestVal) ||
            (botSymbol == 'O' && val < bestVal))
        {
            bestVal = val;
            bestMove = {r, c};
        }
